<template>
  <div class="kpi-panel">
    ...
    <div class="panel-title">
      └ {{ $t('realtime.alarmFeed') }}
      <span v-if="recentAlarms.length" class="title-count"
        >· {{ alarmsStore.activeEvents.length }}</span
      >
    </div>
    <div class="alarm-feed">
      ...
      <div v-for="ev in recentAlarms" :key="ev.id" class="alarm-item" :class="`sev-${ev.severity}`">
        ...
        <span class="badge" :class="`badge-${ev.severity}`">{{
          $t(`alarms.severity.${ev.severity}`).toUpperCase()
        }}</span>
    ...
    <div class="storage">
      <div class="stor-row">
        ...
        <span class="mono"
          >{{ storage.usedGb }} <span class="text-dim">/ {{ storage.totalGb }} GB</span></span
        >
      </div>
      ...
      <div class="stor-row">
        ...
        <span class="mono">{{
          acqStore.timeWindowSec >= 3600
            ? Math.round(acqStore.timeWindowSec / 3600) + ' h'
            : Math.round(acqStore.timeWindowSec / 60) + ' min'
        }}</span>
      </div>
      <div class="stor-row">
...
const writeRate = computed(() =>
  acqStore.isRunning && !acqStore.isPaused
    ? (4.6 + Math.sin(acqStore.elapsedSec * 0.4) * 0.4).toFixed(1)
    : '0.0',
)
