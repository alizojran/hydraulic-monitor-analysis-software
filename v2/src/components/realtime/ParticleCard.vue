<template>
  <div class="pc-card corners">
    ...
    <div class="card-header">
      ...
      <span class="header-meta mono text-dim">ISO 4406 · {{ locale === 'zh' ? '在线监测' : 'Online' }}</span>
    </div>
...
const isoClass = computed(() => isoNum.value >= 19 ? 'text-red' : isoNum.value >= 18 ? 'text-amber' : 'text-cyan')

const cleanClass = computed(() => isoNum.value >= 19 ? 'danger' : isoNum.value >= 18 ? 'warn' : 'ok')
...
.header-meta { font-size: 9.5px; letter-spacing: 0.05em; }

.iso-code {
  display: flex; align-items: baseline; justify-content: center;
  padding: 10px 8px 2px; gap: 2px;
}
.code-val { font-size: 30px; font-weight: 700; line-height: 1; }
.code-sep { font-size: 22px; color: var(--text-2); margin: 0 2px; }

.cleanliness {
  display: flex; align-items: center; justify-content: center; gap: 6px;
  font-size: 11px; font-family: var(--font-mono); padding: 3px 8px 8px;
  letter-spacing: 0.06em;
}
.cleanliness.ok   { color: var(--green); }
.cleanliness.warn { color: var(--amber); }
.cleanliness.danger { color: var(--red); }
.clean-dot { width: 6px; height: 6px; border-radius: 50%; background: currentColor; }
...
.bin-row { display: flex; align-items: center; gap: 6px; font-size: 10.5px; }
.bin-label { color: var(--text-2); min-width: 42px; font-size: 10px; }
.bin-bar-wrap {
  flex: 1; height: 5px; background: var(--bg-2);
  border: 1px solid var(--border); border-radius: 2px; overflow: hidden;
}
.bin-bar { height: 100%; border-radius: 2px; transition: width 0.4s; }
.bin-val { color: var(--text-1); min-width: 48px; text-align: right; }
</style>
