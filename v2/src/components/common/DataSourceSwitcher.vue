  ...
  <div class="ds-switcher">
    ...
    <template v-if="acqStore.dataSource === 'webserial'">
      <template v-if="!serial.isSupported">
        <span class="ds-warn">{{ locale === 'zh' ? '需要 Chrome/Edge' : 'Needs Chrome/Edge' }}</span>
      </template>
    ...
    <template v-if="acqStore.dataSource === 'modbus'">
      ...
      <template v-else>
        <input v-model="modbusHost" class="ds-input mono" placeholder="192.168.1.100" style="width:120px" />
        <input v-model.number="modbusPort" class="ds-input mono" type="number" placeholder="502" style="width:60px" />
...
function srcLabel(src: DataSource) { return LABELS[src] }
...
<style scoped>
.ds-switcher { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.label { font-size: 11px; }
.seg { display: flex; border: 1px solid var(--border); border-radius: var(--r); overflow: hidden; }
.seg-btn {
  background: transparent; border: none; border-radius: 0;
  padding: 4px 10px; font-size: 11px; color: var(--text-2);
...
.ds-select, .ds-input {
  ...
  color: var(--text-0); padding: 4px 7px; border-radius: var(--r);
  font-size: 11px;
}
.ds-input { font-family: var(--font-mono); width: 150px; }
.ds-act-btn {
  padding: 4px 10px; font-size: 11px;
  background: rgba(0,217,255,0.1); border: 1px solid rgba(0,217,255,0.35); color: var(--cyan);
  border-radius: var(--r);
}
.ds-act-btn:hover { background: rgba(0,217,255,0.2); }
.ds-act-btn:disabled { opacity: 0.4; cursor: default; }
.ds-disc-btn {
  padding: 3px 8px; font-size: 11px;
  background: rgba(255,51,85,0.1); border: 1px solid rgba(255,51,85,0.35); color: var(--red);
  border-radius: var(--r);
}
.ds-live { display: flex; align-items: center; gap: 5px; font-size: 10px; color: var(--green); }
.live-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--green); animation: blink 1.2s ease-in-out infinite; }
@keyframes blink { 0%,100%{opacity:1} 50%{opacity:0.2} }
.ds-warn { font-size: 11px; color: var(--amber); }
.ds-err { font-size: 11px; color: var(--red); }
</style>
