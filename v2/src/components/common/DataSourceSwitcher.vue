<template>
  <div class="ds-switcher">
    ...
    <!-- Cloud inline controls -->
    <template v-if="acqStore.dataSource === 'cloud'">
      <template v-if="cloud.status !== 'connected'">
        <input
          v-model="cloudBaseUrl"
          class="ds-input mono cloud-url"
          placeholder="http://localhost:8080"
          @change="cloud.setBaseUrl(cloudBaseUrl)"
          @keydown.enter="connectCloud"
        />
        <input
          v-model="cloudDeviceId"
          class="ds-input mono"
          placeholder="HMAS-001"
          @change="cloud.setDeviceId(cloudDeviceId)"
          @keydown.enter="connectCloud"
        />
        <button
          class="ds-act-btn"
      ...
      <template v-else>
        <span class="ds-live mono">
          <span class="live-dot" />{{ cloud.framesReceived }} fft
        </span>
        <button class="ds-disc-btn" @click="cloud.disconnect()">✕</button>
      </template>
      <span v-if="cloud.errorMsg" class="ds-err">{{ cloud.errorMsg }}</span>
    </template>
  </div>
</template>
...
const SOURCES: DataSource[] = ['simulated', 'csv', 'wav', 'webserial', 'websocket', 'modbus', 'cloud']
...
const cloudBaseUrl = ref(cloud.baseUrl)
const cloudDeviceId = ref(cloud.deviceId)
...
function onSelect(src: DataSource) {
  ...
  if (cloud.status === 'connected') cloud.disconnect()
...
function connectCloud() {
  cloud.setBaseUrl(cloudBaseUrl.value)
  cloud.setDeviceId(cloudDeviceId.value)
  cloud.connect()
}
</script>
