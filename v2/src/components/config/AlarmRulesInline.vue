<template>
  <div class="alarm-rules">
    ...
    <div class="rules-list">
      ...
      <div v-if="alarmsStore.rules.length === 0" class="no-rules text-dim">{{ $t('alarms.noAlarms') }}</div>
    </div>
      ...
      <div class="modal">
        ...
        <div class="form-row">
          ...
          <div class="inline-row">
            <select v-model="form.operator"><option>></option><option>>=</option><option>&lt;</option><option>&lt;=</option></select>
...
function openAdd() {
  ...
  Object.assign(form, { channelId: 'CH01', metric: 'value', operator: '>', threshold: 0, severity: 'warn', label: '', enabled: true })
...
<style scoped>
.alarm-rules { display: flex; flex-direction: column; gap: 10px; max-width: 700px; }
.rules-toolbar { display: flex; gap: 8px; }
...
.rule-label { font-size: 12px; color: var(--text-1); flex: 1; }
.rule-detail { font-size: 10.5px; padding-left: 22px; }
.spacer { flex: 1; }
.action-btn { font-size: 10px; padding: 2px 8px; background: var(--bg-2); border: 1px solid var(--border); border-radius: 2px; color: var(--text-2); }
.action-btn.danger { color: var(--red); border-color: rgba(255,51,85,0.3); }
.no-rules { font-size: 12px; padding: 12px 0; }

.badge { font-size: 9px; padding: 1px 5px; border-radius: 2px; font-family: var(--font-mono); letter-spacing: 0.05em; text-transform: uppercase; }
.badge-high  { background: rgba(255,51,85,0.12); color: var(--red); border: 1px solid rgba(255,51,85,0.3); }
.badge-warn  { background: rgba(255,170,0,0.1);  color: var(--amber); border: 1px solid rgba(255,170,0,0.3); }
.badge-low   { background: rgba(0,217,255,0.08); color: var(--cyan); border: 1px solid rgba(0,217,255,0.2); }
.badge-info  { background: var(--bg-2); color: var(--text-2); border: 1px solid var(--border); }
...
.modal {
  ...
  padding: 20px; min-width: 340px; display: flex; flex-direction: column; gap: 12px;
}
.modal-title { font-size: 13px; color: var(--text-1); font-weight: 600; }
.form-row { display: grid; grid-template-columns: 90px 1fr; align-items: center; gap: 10px; font-size: 12px; }
.form-row label { color: var(--text-2); font-size: 11px; }
.form-row input[type="text"], .form-row input[type="number"], .form-row select { font-size: 12px; padding: 3px 6px; width: 100%; }
