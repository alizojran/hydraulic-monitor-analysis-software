<template>
  <div class="cfg-section">
    ...
    <div
      v-for="(bearing, idx) in dsp.bearings"
      :key="bearing.id"
      class="sub-card"
      :style="idx > 0 ? 'margin-top:8px' : ''"
    >
      <div class="sub-head">
        <input
          class="name-input"
        ...
        <div class="spacer" />
        <button
          v-if="dsp.bearings.length > 1"
          class="danger-sm"
          style="padding: 1px 6px; margin-right: 4px"
          @click="dsp.removeBearing(idx)"
        >
          ×
        </button>
        <div class="toggle-group">
          <button
            :class="{ active: bearing.overlay }"
            @click="!bearing.overlay && dsp.toggleBearingOverlay(idx)"
          >
            ON
          </button>
          <button
      ...
      <div v-if="dsp.allBearingFreqs[idx]" class="freq-preview">
        <span
          v-for="f in freqsFor(idx)"
          :key="f.label"
          class="freq-chip"
          :style="{ color: f.color }"
        >
    ...
    <div class="sub-card" style="margin-top: 10px">
      <div class="sub-head">
        ...
        <div class="toggle-group">
          <button
            :class="{ active: dsp.gearOverlay }"
            :disabled="dsp.gearTeeth <= 0"
            @click="!dsp.gearOverlay && dsp.toggleGearOverlay()"
          >
            ON
          </button>
          <button
      ...
      <div class="cfg-grid">
        ...
        <div v-if="dsp.gearTeeth > 0" class="cfg-row">
          ...
          <span class="mono" style="color: var(--purple)"
            >{{ dsp.gearFreqs.mesh.toFixed(1) }} Hz</span
          >
        </div>
      </div>
    </div>
