<template>
  <div class="profile-switcher">
    ...
    <div class="ps-left">
      ...
      <span class="ps-unsaved" v-if="isDirty">●</span>
    </div>

    <!-- Actions -->
    <div class="ps-actions">
      <button class="ps-btn primary" @click="saveToActive" :title="$t('config.profileSave')">
        💾 {{ $t('config.profileSave') }}
      </button>
      <button class="ps-btn" @click="showNew = true" :title="$t('config.profileNew')">
        + {{ $t('config.profileNew') }}
      </button>
      <button class="ps-btn" @click="cloneActive" :title="$t('config.profileClone')">
        ⧉
      </button>
      <button class="ps-btn" @click="exportActive" :title="$t('config.profileExport')">
        ⤓
      </button>
      <button class="ps-btn" @click="triggerImport" :title="$t('config.profileImport')">
        ⤒
      </button>
      <button v-if="ps.activeProfileName !== DEFAULT_PROFILE_NAME" class="ps-btn danger"
        @click="deleteActive" :title="$t('config.profileDelete')">
        ×
      </button>
    ...
    <div v-if="showNew" class="modal-overlay" @click.self="showNew = false">
      <div class="modal">
        ...
        <input v-model="newName" type="text" :placeholder="$t('config.profileNamePlaceholder')"
          @keydown.enter="confirmNew" class="modal-input" autofocus />
        <div class="modal-actions">
          <button @click="showNew = false">{{ $t('common.cancel') }}</button>
          <button class="primary" @click="confirmNew" :disabled="!newName.trim()">{{ $t('common.confirm') }}</button>
        </div>
      </div>
    </div>
  </div>
...
function cloneActive() {
  ...
  let n = 2
  while (ps.profiles.find(p => p.name === candidate)) { candidate = `${base} (copy ${n++})` }
...
.profile-switcher {
  ...
  padding: 6px 12px; background: var(--bg-1); border-bottom: 1px solid var(--border);
}
.ps-left { display: flex; align-items: center; gap: 6px; }
.ps-label { font-size: 10px; color: var(--text-2); letter-spacing: 0.06em; text-transform: uppercase; white-space: nowrap; }
.ps-select-wrap { position: relative; }
.ps-select { font-size: 11px; padding: 3px 8px; height: 24px; min-width: 140px; background: var(--bg-2); border: 1px solid var(--cyan); border-radius: 2px; color: var(--text-1); }
.ps-unsaved { color: var(--amber); font-size: 14px; line-height: 1; }
.ps-actions { display: flex; gap: 4px; }
.ps-btn {
  font-size: 11px; padding: 2px 9px; height: 24px;
  background: var(--bg-2); border: 1px solid var(--border); border-radius: 2px; color: var(--text-2);
  white-space: nowrap;
}
.ps-btn:hover { border-color: var(--text-2); color: var(--text-1); }
.ps-btn.primary { background: rgba(0,217,255,0.1); border-color: var(--cyan); color: var(--cyan); }
.ps-btn.primary:hover { background: rgba(0,217,255,0.18); }
.ps-btn.danger { color: var(--red); border-color: rgba(255,51,85,0.3); }
...
.modal-input {
  font-size: 13px; padding: 6px 10px; background: var(--bg-2);
  border: 1px solid var(--border); border-radius: 2px; color: var(--text-1); outline: none; width: 100%;
}
.modal-input:focus { border-color: var(--cyan); }
.modal-actions { display: flex; justify-content: flex-end; gap: 8px; }
.primary { background: rgba(0,217,255,0.12); border: 1px solid var(--cyan); color: var(--cyan); padding: 4px 14px; border-radius: 2px; font-size: 12px; }
