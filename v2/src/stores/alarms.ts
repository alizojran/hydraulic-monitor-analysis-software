...
export const useAlarmsStore = defineStore('alarms', () => {
  ...
  const activeSeverity = computed<AlarmSeverity | null>(() => {
    ...
    return activeEvents.value.reduce<AlarmSeverity>((max, e) => SEV_ORDER[e.severity] > SEV_ORDER[max] ? e.severity : max, 'info')
  })

  function saveRules() { localStorage.setItem('daq-alarm-rules', JSON.stringify(rules.value)) }
  function saveEvents() { localStorage.setItem('daq-alarm-events', JSON.stringify(events.value.slice(-1000))) }
  ...
  function updateRule(id: string, patch: Partial<AlarmRule>) {
    ...
    if (idx >= 0) { rules.value[idx] = { ...rules.value[idx], ...patch }; saveRules() }
  }
  function deleteRule(id: string) { rules.value = rules.value.filter(r => r.id !== id); saveRules() }
  function toggleRule(id: string) { updateRule(id, { enabled: !rules.value.find(r => r.id === id)?.enabled }) }

  function acknowledge(id: string) {
    const e = events.value.find(ev => ev.id === id)
    if (e) { e.status = 'acknowledged'; saveEvents() }
  }
  ...
  function evaluateFrame(frame: SampleFrame, channelRms: Record<string, number>, channelPeak: Record<string, number>) {
    ...
    for (const rule of rules.value) {
      ...
      const val = rule.metric === 'value' ? (frame.channels.get(rule.channelId) ?? 0)
        : rule.metric === 'rms' ? (channelRms[rule.channelId] ?? 0)
        : (channelPeak[rule.channelId] ?? 0)

      const triggered = rule.operator === '>' ? val > rule.threshold
      ...
      if (triggered && !wasTriggered) {
        ...
        const activeEv = [...events.value].reverse().find(e => e.ruleId === rule.id && e.status !== 'resolved')
        if (activeEv) { activeEv.status = 'resolved'; saveEvents() }
      }
